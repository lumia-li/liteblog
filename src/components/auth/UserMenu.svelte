...
function openAuthModal(view: "login" | "reg-email") {
	closeMenu();
	...
	{:else}
		...
		{#if open}
			...
			>
				...
				<button type="button" class="user-dropdown-item user-dropdown-register" role="menuitem" on:click={() => openAuthModal("reg-email")}>
				...
				<a href="/api/auth/qq-login" class="user-dropdown-item" role="menuitem" on:click={closeMenu}>
					<img src="/qq-logo.png" alt="" width="20" height="20" />
					使用 QQ 登录
				</a>
