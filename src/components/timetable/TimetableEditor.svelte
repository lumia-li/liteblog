...
export let apiUrl = "/api/timetable";
export let autoSyncWeekToServer = true;
export let autoDetectWeekOnLoad = true;
export let keepLocalWeekControl = false;
...
function resolveApiUrl(input: string): string {
	...
	return normalized || "/api/timetable";
}

let timetableApiUrl = "/api/timetable";
$: timetableApiUrl = resolveApiUrl(apiUrl);
const DAY_ORDER: DayKey[] = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"];
const DISPLAY_DAYS: DisplayDayKey[] = [
	"mon",
	"tue",
	"wed",
	"thu",
...
const IMPORT_PERIOD_GROUPS: Array<{
	label: string;
	slots: number[];
	time: string;
}> = [
...
const DEFAULT_PERIODS: PeriodItem[] = IMPORT_PERIOD_GROUPS.map(
	(item, index) => ({
		id: `p${index + 1}`,
		label: item.label,
		time: item.time,
	...
	if (Array.isArray(input)) {
		return Array.from(
			new Set(
				input
					.map((item) =>
						typeof item === "string" ? normalizeSearchTag(item) : "",
					)
					.filter(Boolean),
			),
		);
...
function normalizeCourses(input: unknown): Record<string, CourseItem[]> {
	...
	for (const [key, value] of Object.entries(input as Record<string, unknown>)) {
		if (Array.isArray(value)) {
			normalized[key] = value.filter(Boolean).map((item) => {
				...
				return {
					...
					location:
						typeof rawItem.location === "string" ? rawItem.location : "",
...
function cloneCoursesMap(
	input: Record<string, CourseItem[]>,
...
function normalizeImportedPeriods(
	...
	const currentLabels = sourcePeriods.map((item) =>
		normalizePeriodGroupLabel(item.label),
	);
	...
	for (
		let periodIndex = 0;
		periodIndex < sourcePeriods.length;
		periodIndex += 1
	) {
	...
	if (!slotToPeriodIds.size)
		return { periods: sourcePeriods, courses: sourceCourses };
	...
	for (let i = 0; i < IMPORT_PERIOD_GROUPS.length; i += 1) {
		...
		const groupPeriodIds = group.slots.flatMap(
			(slot) => slotToPeriodIds.get(slot) || [],
		);
...
function applyParsedState(
	parsed: Partial<TimetableState>,
	options: {
		preserveCurrentWeek?: boolean;
		preserveTermStartDate?: boolean;
	} = {},
) {
	if (typeof parsed.termName === "string" && parsed.termName.trim())
		termName = parsed.termName;
	if (typeof parsed.totalWeeks === "number" && parsed.totalWeeks > 0)
		totalWeeks = parsed.totalWeeks;
	if (
		!options.preserveCurrentWeek &&
	...
	if (
		!options.preserveTermStartDate &&
		typeof parsed.termStartDate === "string"
	) {
		termStartDate = parsed.termStartDate;
	}
	const parsedWeeklySheets: Record<
		string,
		{ periods: PeriodItem[]; courses: Record<string, CourseItem[]> }
	> = {};
	...
	if (rawWeeklySheets && typeof rawWeeklySheets === "object") {
		for (const [weekKey, value] of Object.entries(
			rawWeeklySheets as Record<string, unknown>,
		)) {
	...
	if (weeklyMode) {
		const knownWeeks = Object.keys(parsedWeeklySheets)
			.map((key) => Number(key))
...
//
async function loadState(targetWeek?: number) {
	...
	try {
		const hasTargetWeek =
			...
			targetWeek > 0;
		const requestUrl = new URL(timetableApiUrl, window.location.origin);
		if (hasTargetWeek) {
			requestUrl.searchParams.set("week", String(Math.floor(targetWeek)));
		}
		...
		const result = (await response
			.json()
			.catch(() => ({}))) as TimetableApiGetResponse;
		if (!response.ok) {
			throw new Error(
		...
		if (result.state && typeof result.state === "object") {
			applyParsedState(result.state, {
				preserveCurrentWeek: keepLocalWeekControl && hasTargetWeek,
				preserveTermStartDate: keepLocalWeekControl,
			});
			if (hasTargetWeek) {
				currentWeek = Math.max(1, Math.min(totalWeeks, Math.floor(targetWeek)));
			}
			importMessage = "";
			importError = "";
			return;
...
onMount(() => {
	...
	void (async () => {
		try {
			...
			if (autoDetectWeekOnLoad) {
				await syncDetectedWeekOnLoad();
			}
		} catch (error) {
			importError = error instanceof Error ? error.message : "初始化课程表失败";
		} finally {
			hasLoadedState = true;
		}
	})();
...
$: isViewingDetectedWeek =
	detectedCurrentWeek !== null && currentWeek === detectedCurrentWeek;
...
async function syncDetectedWeekOnLoad() {
	...
	syncViewFromWeekly();
	if (autoSyncWeekToServer && hasDevCredential()) {
...
function addPeriod() {
	...
	periods = [
		...periods,
		{
			id: `p${Date.now()}_${next}`,
			label: `第 ${next * 2 - 1}-${next * 2} 节`,
			time: "",
		},
	];
	writeCurrentWeekToWeekly();
...
function openEditor(periodId: string, day: DayKey) {
	...
	const activeEntry =
		(weeklyMode
			? current?.[0]
			: current?.find((item) => parseWeekInRange(item.weeks, currentWeek))) ||
		current?.[0] ||
		null;
...
function setEditingColor(color: string) {
	editingForm = { ...editingForm, color };
}
...
function parseCourseText(cellValue: string): CourseItem[] {
	...
	const isWeekOnlyLine = (line: string): boolean => {
		...
		return /^(?:第)?\d{1,2}(?:[-~]\d{1,2})?(?:[，,]\d{1,2}(?:[-~]\d{1,2})?)*周(?:\([单双]\))?$/.test(
			compact,
		);
	};
	const normalizeCourseName = (line: string): string => {
		const text = line
			.replace(weekExpr, "")
			.replace(
				/(?:周|星期|礼拜)[一二三四五六日天](?:第?\d{1,2}(?:[-~]\d{1,2})?节)?/g,
				"",
			)
	...
	const isLikelyLocationLine = (line: string): boolean => {
		...
		return /楼|室|馆|场|区|实验室|教室|运动场|\(|（|\d{3,4}|J\d{3}|SG\d{3}/.test(
			compact,
		);
	};
	...
	const isCourseTitleLine = (line: string): boolean => {
		...
		if (
			/^(?:周|星期|礼拜)[一二三四五六日天](?:第?\d{1,2}(?:[-~]\d{1,2})?节)?$/.test(
				compact,
			)
		) {
			return false;
		...
		if (
			/^(教室|地点|上课地点|教室地点|房间|教师|老师|任课教师|授课教师)/.test(
				line,
			)
		)
			return false;
	...
	const parseBlock = (blockLines: string[]): CourseItem | null => {
		...
		let course =
			blockLines.find((line) => isCourseTitleLine(line)) || blockLines[0] || "";
		let weeks = "";
		let location = "";
		let teacher = "";
		...
		for (const line of blockLines) {
			if (/^(教室|地点|上课地点|教室地点|房间)\s*[:：]/.test(line)) {
				location = line
					.replace(/^(教室|地点|上课地点|教室地点|房间)\s*[:：]\s*/, "")
					.trim();
				continue;
			}
			if (/^(教师|老师|任课教师|授课教师)\s*[:：]/.test(line)) {
				teacher = line
					.replace(/^(教师|老师|任课教师|授课教师)\s*[:：]\s*/, "")
					.trim();
			}
		}

		if (!location) {
			const loc = joined.match(
				/(?:教室|地点|上课地点|房间)\s*[:：]\s*([^\n,，;；]+)/,
			);
			if (loc?.[1]) location = loc[1].trim();
		}
		if (!teacher) {
			const tch = joined.match(
				/(?:教师|老师|任课教师|授课教师)\s*[:：]\s*([^\n,，;；]+)/,
			);
...
function buildSearchState(keyword: string): SearchState {
	...
	const dayGroups = DISPLAY_DAYS.map((day) => {
	...
	try {
		...
		for (const sheetName of workbook.SheetNames) {
			...
			for (let r = header.rowIndex + 1; r < rows.length; r += 1) {
				...
				const timeCandidate = (row[header.periodCol + 1] || "")
					.toString()
					.trim();
				nextPeriods.push({
					id: periodId,
					label:
						periodRaw || `第 ${periodIndex * 2 + 1}-${periodIndex * 2 + 2} 节`,
		...
		if (weeklySheetEntries.length >= 2) {
			...
			const maxWeekNo = weeklySheetEntries.reduce(
				(max, item) => Math.max(max, item.weekNo || 0),
				1,
			);
			weeklyMode = true;
			...
			termName =
				extractTermName(
					weeklySheetEntries[0].sheetName,
					weeklySheetEntries[0].title,
				) || termName;
		...
		termName =
			extractTermName(first.sheetName, first.title) ||
			first.sheetName ||
			termName;
		if (first.weekNo) {
...
<section class="tt-root">
	...
	<div class="week-row">
		...
		<div class="week-label">{hasLoadedState ? `第 ${currentWeek} 周` : "周次加载中..."}</div>
								...
								{#if item}
									...
									>
										...
										>
											...
											<p class="course-meta course-location">教室: {item.location || "未填写"}</p>
											<p class="course-meta course-teacher">教师: {item.teacher || "未填写"}</p>
										</div>
									</button>
								{:else}
			...
			<div>
				...
				<div class="color-row">
					<input
						...
						aria-label="自定义颜色"
						on:input={(event) => setEditingColor((event.currentTarget as HTMLInputElement).value)}
					/>
					{#each CARD_COLORS as color}
						<button
							...
							on:touchstart|preventDefault={() => setEditingColor(color)}
							on:click={() => setEditingColor(color)}
						>
