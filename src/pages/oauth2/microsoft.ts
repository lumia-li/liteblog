...
export const GET: APIRoute = async ({ request }) => {
	...
	try {
		...
		const tokenResult = await exchangeMicrosoftCodeForToken(code, config);
		if (!tokenResult.success) {
			...
			return redirectResponse(
				`/?auth_error=token_exchange_failed:${encodeURIComponent(tokenResult.error)}`,
			);
		}

		const user = await fetchMicrosoftUserInfo(tokenResult.data.access_token);
		if (!user?.id) {
